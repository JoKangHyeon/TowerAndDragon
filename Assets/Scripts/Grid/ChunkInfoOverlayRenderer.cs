...
public class ChunkInfoOverlayRenderer : MonoBehaviour
{
    ...
    public void Refresh(IReadOnlyList<Vector2Int> chunkCoords)
    {
        for (int i = 0; i < chunkCoords.Count; i++)
        {
            ...
            BuildResourceIconEntries(coord);
            card.Setup(terrainSprite, _populationIcon, string.Format(PLUS_VALUE_FORMAT, populationReward), _resourceIconBuffer);
        }
    ...
    // 이 청크를 점령했을 때 생산 가능해지는 해금 자원 아이콘을 재사용 버퍼에 채운다.
