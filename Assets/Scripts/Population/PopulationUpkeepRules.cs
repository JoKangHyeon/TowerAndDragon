...
public static class PopulationUpkeepRules
{
    ...
    // float 배율을 double로 넓힐 때 생기는 표현 오차 흡수분. 예를 들어 0.6f는 double에서
    // 0.60000002...가 되어 인구 50에 곱하면 30.000001이 되고, 그대로 올림하면 31이 된다
    // (정확히 30이어야 한다). 올림 직전에 이 값을 빼서 "사실상 정수"인 값이 한 칸 올라가지 않게 한다.
    ...
    /// 1명당 값을 정수로 유지하면 big_appetite 1단계(x1.5)가 올림에서 2, 2단계(x2)도 2가 되어
    /// <b>두 단계가 완전히 같아진다</b>(2단계가 0점짜리 뮤테이터가 된다). 내림을 고르면 반대로
    /// 1단계가 아무 효과도 없다. 정수 1명당 값으로는 x1.5를 표현할 방법이 아예 없다.
    ...
    ///
    /// 올림을 고르는 이유: 내림이면 소수부가 통째로 버려져 인구가 적은 초반에 x1.5가 다시
