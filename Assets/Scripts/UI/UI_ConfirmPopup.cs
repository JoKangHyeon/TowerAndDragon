...
/// 지금은 밤 진입 전 굶주림 경고가 유일한 사용처지만, 같은 성격의 경고를 붙일 때
/// <see cref="Open"/> 한 줄이면 된다(UI_SlotConfirmPopup이 슬롯 번호에 묶여 있어 재사용할 수 없었다).
///
...
public class UI_ConfirmPopup : MonoBehaviour
{
    ...
    /// <summary>
    /// 확인을 띄운다. YES를 누르면 <paramref name="onConfirmed"/>가 실행되고, NO를 누르면 아무 일도 없다.
    /// </summary>
    ...
    {
        if (_messageText == null || string.IsNullOrEmpty(_messageLocKey))
        {
            return;
        }

        string message = StringTable.GetString(_messageLocKey);

        _messageText.text = _messageArgs == null || _messageArgs.Length == 0
            ? message
            : string.Format(message, _messageArgs);
    }
