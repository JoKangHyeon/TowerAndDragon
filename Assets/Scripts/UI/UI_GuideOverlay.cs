...
/// <summary>
/// 가이드 연출 전담. "어디에 무슨 말을 띄울지"만 지시받고 새끼용 도메인은 모른다.
...
/// 단일 인스턴스로 쓰는 것을 전제한다 - 그래야 안내끼리 겹치지 않는다.
/// </summary>
public class UI_GuideOverlay : MonoBehaviour
{
    ...
    [Tooltip("말풍선 아트를 담은 루트. 위치는 코드가 건드리지 않으므로 씬에서 놓은 자리에 그대로 뜬다 " +
             "(토스트처럼 화면 중앙 살짝 위 권장). _overlayRoot의 자식이어야 한다.")]
    ...
    {
        if (!_isShowing || _bubbleText == null)
        {
            return;
        }
    ...
    /// </summary>
    public void Show(RectTransform target, string locKey, bool blocksInput, params object[] args)
    {
