...
- 현재 구현 상태: `Docs/구현현황.md` — 코드가 지금 어떻게 동작하는지의 단일 진입점
  (기획서와 다른 점·알려진 공백·씬 배선 확인 결과가 여기 정리돼 있다)
- 자원 흐름: `Docs/자원순환_플로우차트.mermaid` (렌더본: 같은 이름 .svg)
- 타워 이펙트: `Docs/타워이펙트_작업노트.md` — 발사체·명중 이펙트를 손대기 전에 반드시 읽을 것
  (현재 배선, 이 프로젝트 카메라에서 이펙트가 깨지는 이유, 겪은 함정들. 작업 완료 후 정리 예정)
...
- 핵심 시스템: 어미용(5속성) / 새끼용 / 인구 배분(생산·방어·점령) /
  ...
  자원(기본4 + 특화4 + 슬라임) / 연구 3갈래(타워·생산시설·편의)
...
### 새 모달 창에는 `UI_CameraInputBlocker`를 붙입니다

창이 떠 있는 동안에도 화면 가장자리로 마우스를 가져가면 카메라가 계속 움직이는 문제가 있었습니다.
`Assets/Scripts/UI/UI_CameraInputBlocker.cs`를 **실제로 켜고 꺼지는 오브젝트**(자기 자신을 끄는 창은
그 오브젝트에, 자식 `_windowRoot`만 끄는 창은 그 자식에)에 붙이면 켜져 있는 동안 카메라의
WASD·엣지 스크롤·좌드래그·휠 줌·북마크가 전부 막힙니다. `OnEnable`/`OnDisable`에서 자동으로
`CameraInputBlockRegistry`에 등록/해제되므로 코드 작성이나 별도 배선은 필요 없습니다.

**붙이면 안 되는 곳**: 창이 떠 있는 채로 월드를 조작해야 하는 모드(건설·점령·인구 배치·워커·스킬
타겟팅·새끼용 인벤토리와, 그리드 선택에 종속된 정보창들). 이 모드들은 카메라를 움직이며 쓰는 것이
정상 동작이므로, 여기 붙이면 카메라가 죽습니다.
